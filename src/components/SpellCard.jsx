...
export default function SpellCard({ spell, lore }) {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: `1px solid ${expanded ? lore.border : 'var(--border)'}`, borderRadius: 6, overflow: 'hidden', boxShadow: 'var(--shadow-card)' }}>
      <button onClick={() => setExpanded(e => !e)} aria-expanded={expanded}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', width: '100%', background: expanded ? lore.bg : 'var(--bg-card)', border: 'none', cursor: 'pointer', textAlign: 'left', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'Cinzel,serif', fontSize: 14, fontWeight: 700, color: expanded ? lore.color : 'var(--text-primary)' }}>{spell.name}</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          <div style={{ fontFamily: 'Cinzel,serif', fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.08em', textAlign: 'right' }}>Your Hero Phase</div>
          <span style={{ color: 'var(--text-dim)', fontSize: 14 }} aria-hidden="true">{expanded ? '▲' : '▼'}</span>
        </div>
      </button>
      {expanded && (
        <div style={{ padding: '14px 16px 16px', borderTop: `1px solid ${lore.border}` }}>
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ fontSize: 14, color: 'var(--text-body)', lineHeight: 1.65 }}>{spell.declare}</div>
          </div>
          <div>
            ...
            <div style={{ fontSize: 14, color: 'var(--text-body)', lineHeight: 1.65 }}>{spell.effect}</div>
          </div>
        </div>
      )}
    </div>
